    ...
    const getTeamsAndGroups = async () => {
        ...
        try {
            ...
            if (groupsResponse.ok) {
                ...
                newGroups = groups.map((group, index) => ({
                    id: group.id ?? `temp-${index}`, // ✅ Pokud chybí ID, přidáme dočasné
                    name: group.name,
                ...
                newGroups = Array.from({ length: 4 }, (_, index) => ({
                    id: -(index + 1), // Use a temporary negative number as ID
                    name: `Skupina ${String.fromCharCode(65 + index)}`,
                    teams: [],
                }));
            } else {
        ...
        setGroups(
            variant.groups.map((group) => ({
                ...
                teams: group.teams.map((team) => ({
                    ...team,
                    strength:
                        teamDraws.find((t) => t.id === team.id)?.strength ?? 1,
                    isGirls:
                        teamDraws.find((t) => t.id === team.id)?.isGirls ??
                        false,
                })),
            }))
        );
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        {groupVariants.length === 0 ? (
                            <Button
                                ...
                                disabled={
                                    selectedVariant !== null || hasTeamsInGroups
                                }
                            >
                                {hasTeamsInGroups
                                    ? "Vyčistěte skupiny"
                        ...
                        <>
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <Input
                                    ...
                                    value={groupCount}
                                    onChange={(e) =>
                                        updateGroupCount(Number(e.target.value))
                                    }
                            ...
                            {selectedVariant ? (
                                <div>
                                    <h3 className="text-lg font-bold">
                                        Varianta ({selectedVariant.groupCount}{" "}
                                        skupin)
                                    </h3>
                                    <p>
                                        Celkem zápasů:{" "}
                                        {selectedVariant.totalMatches} (
                                        Minimálně{" "}
                                        {selectedVariant.minMatchesPerTeam} na
                                        tým)
                                    </p>
                                ...
                                groupVariants.map((variant, index) => (
                                    ...
                                    >
                                        ...
                                        >
                                            Varianta {index + 1} (
                                            {variant.groupCount} skupin)
                                        </h3>
                                        <p>
                                            Celkem zápasů:{" "}
                                            {variant.totalMatches} ( Minimálně{" "}
